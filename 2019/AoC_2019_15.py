...
def recur(x, y, i, relative_base):
  ...
  for direction in range(1, 5):
    if direction == 1:
      ...
      new_pos = (x + 1, y)
    if new_pos in seen or new_pos in walls:
      continue
    result, new_i, new_relative_base = move(direction, i, relative_base)
    if result == 0:
      walls.add(new_pos)
    elif result == 1:
      moved =  recur(new_pos[0], new_pos[1], new_i, new_relative_base)
      if moved[0]:
        return moved[0], moved[1], moved[2] + 1
      move(backtrack[direction], i, relative_base)
    else:
      return True, new_pos, 1
  return False, (x, y), 0
